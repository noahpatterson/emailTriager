...
Public Vercel + Neon for the deployment spike is declared under [`services/`](../../services/README.md) (Terraform). Fill `services/.env` from `services/.env.example` before `terraform apply`. **Spike mode** (`TF_VAR_deployment_mode=spike`, default) requires `TF_VAR_allowed_cidrs` with your public IP; Terraform declares a Vercel Firewall rule and `proxy.ts` enforces the same list. **Demo mode** (`TF_VAR_deployment_mode=demo`) clears that lock, sets `APP_PROFILE=demo`, and points runtime `DATABASE_URL` at `emailtriager_app` (NOBYPASSRLS) — see root README “Unlock public demo on Vercel”. After deploy, `GET /api/health` must return `{"ok":true}` with no secrets. RLS `SET LOCAL` scoping is covered by `src/test/rls-session-probe.test.ts` when `TEST_DATABASE_URL` points at the Neon pooler. Do not enable Neon IP Allow for “my IP only” on this stack — Vercel serverless would lose DB access. For demo, also schedule `bun run demo:cleanup` against the Neon owner URI so expired visitors do not accumulate.
