...
ALTER TABLE "message_processing" ADD CONSTRAINT "message_processing_outcome_check" CHECK ("message_processing"."outcome" IN ('priority','review','new','unmatched','protected','failed','blocked'));--> statement-breakpoint
ALTER TABLE "gmail_message_state" ADD CONSTRAINT "gmail_message_state_outcome_check" CHECK ("outcome" IN ('priority','review','new','unmatched','protected','failed','blocked'));--> statement-breakpoint
